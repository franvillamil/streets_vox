...
In these models, we only compare municipalities that had at least one street with Francoist name at the beginning of the period.\footnote{Moreover, in order to keep the same sample across all the models, we only include municipalities in which Vox participated in 2016 elections.}
...
Figure~\ref{fig:par_trends_norm} shows normalized electoral trends among the control and treatment groups for PP and Vox, and we include in the appendix much more detailed models analyzing the parallel trends assumption for Vox, PP, and PSOE.\footnote{For the Vox analyses, we only have data on one `pre-treatment' elections, December 2015. However, given than most votes for Vox arguably came from former PP voters, the test of parallel trends for PP vote should also help aleviate concerns among the population of interest.}
